...
// 存储共享的动态 UUID 和时间戳
let sharedUUID = {
	value: 'ef9d104e-ca0e-420e-4202-ba4b-a0afb969c747', // 默认初始值
	timestamp: Date.now()
};

// 检查并更新共享 UUID（有效期一小时）
function refreshSharedUUID() {
	const currentTime = Date.now();
	const oneHour = 60 * 60 * 1000; // 一小时（毫秒）

	if (!sharedUUID.value || (currentTime - sharedUUID.timestamp) > oneHour) {
		sharedUUID = {
			value: crypto.randomUUID(), // 生成新的标准 UUID
			timestamp: currentTime
		};
		console.log(`新动态 UUID 生成: ${sharedUUID.value}`);
	}
...
export default {
	async fetch(req, env) {
		// 优先使用环境变量 UUID，没有则使用动态生成的
		let currentUUID = env.UUID || refreshSharedUUID();

		const url = new URL(req.url);

		// 新增：访问 /sfu 路径返回当前 UUID
		if (url.pathname === '/sfu') {
		...
		if (req.headers.get('Upgrade')?.toLowerCase() === 'websocket') {
			...
			new ReadableStream({
				...
				async write(data) {
					...
					const expectedUUID = currentUUID.replace(/-/g, '');
					let valid = true;
					for (let i = 0; i < 16; i++) {
